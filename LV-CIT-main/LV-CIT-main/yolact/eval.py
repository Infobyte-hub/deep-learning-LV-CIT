...
def parse_args(argv=None):
    ...
    parser.add_argument('--labelspace', default="COCO", dest='labelspace',
                        help='Which dataset to use.')
    parser.add_argument('--matting', default=True, type=str2bool, dest='matting',
                        help='weather to do matting or not.')
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str='',
    ...
    def masks_repair(mask):
        # mask = cv2.morphologyEx(
        #     mask,
        #     op=cv2.MORPH_CLOSE,
        #     kernel=np.ones(shape=[int(mask.shape[0]/500)+1, int(mask.shape[1]/500)+1], dtype=np.uint8),
        #     iterations=2)
        # mask = cv2.morphologyEx(
        #     mask,
        #     op=cv2.MORPH_OPEN,
    ...
    if matting:
        if not args.display_masks and cfg.eval_mask_branch and num_dets_to_consider > 0:
            masks = masks[:num_dets_to_consider, :, :, None]
        # if cfg.eval_mask_branch and num_dets_to_consider > 0:
        #     masks_all = masks_repair((masks[0]*255).byte().cpu().numpy())
        #     for i in range(1, num_dets_to_consider):
        #         masks_all = np.bitwise_or(masks_all, masks_repair((masks[i]*255).byte().cpu().numpy()))
        #     # img_numpy = np.bitwise_or(img_numpy, (255 - masks_all))
        # img_numpys = [("all", np.bitwise_or(img_numpy, (255 - masks_all)))]
        ...
        for j in reversed(range(num_dets_to_consider)):
            ...
            if x2 - x1 > y2 - y1:
                final_x = 640
                final_y = int(640 / (x2 - x1) * (y2 - y1))
            else:
                final_x = int(640 / (y2 - y1) * (x2 - x1))
                final_y = 640
            mask = masks_repair((masks[j] * 255).byte().cpu().numpy())
            img_numpys.append([
                cfg.dataset.class_names[classes[j]],
                cv2.resize(cv2.merge(cv2.split(img_numpy) + [mask])[y1:y2, x1:x2], [final_x, final_y])
            ])
        return img_numpys
    else:
        return [["", img_numpy]]
...
def evalimage(net: Yolact, path: str, save_path: str = None):
    def check_class(l1, l2):
        if l1 == l2:
            return True
        if args.labelspace.upper() == "VOC":
            class_map = {
                'aeroplane': ["airplane"],
                'bicycle': ['bicycle'],
                'bird': ['bird'],
                ...
                'sheep': ['sheep', 'cow'],
                'sofa': ['sofa', 'bench', 'couch', 'chair', 'bed'],
                'train': ['train', 'truck'],
                'tv': ['tv', 'microwave', 'toaster', 'oven', 'refrigerator'],
            }
        elif args.labelspace.upper() == "COCO":
            class_map = {
                "airplane": ["airplane"],
                "apple": ["apple"],
                "backpack": ["backpack"],
                ...
                "bowl": ["bowl"],
                "broccoli": ["broccoli"],
                "bus": ["bus", "truck", "car", "train"],
                "cake": ["cake"],
                "car": ["bus", "truck", "car", "train"],
                "carrot": ["carrot"],
                "cat": ["cat", "dog"],
                "cell phone": ["cell phone"],
                "chair": ["bench", "couch", "chair"],
                "clock": ["clock"],
                "couch": ["bench", "couch", "chair"],
                "cow": ["cow", "cat", "dog"],
                "cup": ["cup"],
                "dining table": ["dining table"],
                "dog": ["cat", "dog"],
                ...
                "frisbee": ["frisbee"],
                "giraffe": ["giraffe"],
                "hair dryer": ["hair drier", "bottle", "cup", "remote", "umbrella", "toothbrush"],
                "handbag": ["handbag"],
                "horse": ["horse"],
                "hot dog": ["hot dog"],
                "keyboard": ["keyboard"],
                ...
                "knife": ["knife"],
                "laptop": ["laptop"],
                "microwave": ["tv", "microwave", "toaster", "oven", "refrigerator"],
                "motorcycle": ["motorcycle"],
                "mouse": ["mouse"],
                "orange": ["orange"],
                "oven": ["tv", "microwave", "toaster", "oven", "refrigerator"],
                "parking meter": ["parking meter"],
                "person": ["person"],
                "pizza": ["pizza"],
                "potted plant": ["potted plant", "vase"],
                "refrigerator": ["tv", "microwave", "toaster", "oven", "refrigerator"],
                "remote": ["remote"],
                "sandwich": ["sandwich"],
                "scissors": ["scissors"],
                "sheep": ["sheep", "cow"],
                ...
                "tie": ["tie"],
                "toaster": ["tv", "microwave", "toaster", "oven", "refrigerator"],
                "toilet": ["toilet"],
                "toothbrush": ["toothbrush"],
                "traffic light": ["traffic light"],
                "train": ["bus", "truck", "car", "train"],
                "truck": ["bus", "truck", "car", "train"],
                "tv": ["tv", "microwave", "toaster", "oven", "refrigerator"],
                "umbrella": ["umbrella"],
        ...
        if l2 in class_map.keys():
            return l1 in class_map[l2]
        else:
            raise Exception(f"Unknown class {l1, l2}")
    ...
    try:
        img_numpys = prep_display(preds, frame, None, None, undo_transform=False, matting=args.matting)
    ...
    try:
        for _class, img_numpy in img_numpys:
            save_pathes = save_path.split(".")
            cv2.imwrite(f"{'.'.join(save_pathes[0:-1])}-{_class}.png", img_numpy)
            # if check_class(_class, save_path.split("\\")[-2]):
            ...
            #         # cv2.imwrite(os.path.join(filepath, filename), img_numpy)
    except Exception as e:
        print(path)
        with open("error.txt", "a") as f:
            f.write(path)
...
def evalimages(net: Yolact, input_folder: str, output_folder: str):
    ...
    for dir_path, dir_list, file_list in os.walk(input_folder):
        for file_name in file_list:
            input_path = os.path.join(dir_path, file_name)
            output_dir = dir_path.replace(input_folder, output_folder)
            if not os.path.exists(output_dir):
                os.makedirs(output_dir)
            output_path = os.path.join(output_dir, file_name)

            evalimage(net, input_path, output_path)
            print(input_path + ' -> ' + os.path.join(output_dir, ".".join(file_name.split(".")[:-1]) + "-*" + file_name.split(".")[-1]))
    print('Done.')
